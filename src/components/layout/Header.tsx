...
function MobileMenu({ onClose }: { onClose: () => void }) {
  ...
  return createPortal(
    <div className={`fixed inset-0 bg-white ${overlayClass}`} style={{ zIndex: 9999 }}>
      {/* Close button */}
      <button
        className={`absolute top-4 right-5 w-6 h-5 flex items-center justify-center z-10 ${contentClass}`}
        onClick={handleClose}
        aria-label="Cerrar menú"
      >
        <svg className="w-[18px] h-[18px] text-[#222]" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
      ...
      <div className={`flex flex-col items-center justify-center h-full gap-8 ${contentClass}`}>
        ...
        <div className="flex items-center gap-4 mt-2">
          <a href={SITE.instagram} target="_blank" rel="noopener noreferrer" className="text-[#999] hover:text-[#222] transition-colors" aria-label="Instagram">
            <svg className="w-[15px] h-[15px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.3}>
  ...
  return (
    <>
      ...
      <aside className="hidden lg:flex fixed top-0 left-0 w-[233px] h-screen z-40 flex-col items-start pt-10 pl-10">
        ...
        <div className="flex items-center gap-3 mt-4 sidebar-fade" style={{ animationDelay: '0.4s' }}>
          <a href={SITE.instagram} target="_blank" rel="noopener noreferrer" className="text-[#999] hover:text-[#222] transition-colors" aria-label="Instagram">
            <svg className="w-[15px] h-[15px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.3}>
              <rect x="2" y="2" width="20" height="20" rx="5" />
              <circle cx="12" cy="12" r="5" />
